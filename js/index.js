...
fetch("js/songs.json")
  ...
  .then((data) => {
    ...
    prev.addEventListener("click", () => {
      ...
      document.getElementById( activeIndexId + "-num"
      ).innerHTML = `<img class="icon" src="./images/icons8-speaker-24.png" alt="" />`;
...
if (shuffleOn) {
      ...
      if (activeIndex >= 10) {
       ...
       activeIndex + 1 > 9? activeIndexId = "" + (activeIndex + 1) : activeIndexId = "0" + (activeIndex + 1);
      }
      document.getElementById( activeIndexId).classList.add("active");
      document.getElementById( activeIndexId + "-num"
      ).innerHTML = `<img class="icon" src="./images/icons8-speaker-24.png" alt="" />`;
...
audio.addEventListener("timeupdate", (e) => {
  ...
  let rest = e.srcElement.duration - e.srcElement.currentTime;
  progress.style.left = `calc(
    ${(e.srcElement.currentTime / e.srcElement.duration) * 100}% - 4px )` ;
  ...
  if (rest < 10) {
    ...
    timePlayed.innerText = "- " + (Math.floor(rest / 60) + ":0" + Math.floor(rest % 60));
  } else {
       let output = "- " + Math.floor(rest / 60) + ":" + Math.floor(rest % 60);
       if ( rest < e.srcElement.duration){
     timePlayed.innerText = output;}
  }
...
function shuffling() {
  if (shuffleOn) {
    ...
    random[0] + 1 == datumId.length ? activeIndex = random[0] - 1 : activeIndex = random[0] + 1;
    console.log("single");
  } else {activeIndex = random[0];}
  } 
  //console.log(activeIndex);
}

show.addEventListener("click", () => 
{if (show.classList.contains("showing") == false){container.style.height = 'max-content';
show.style.transform = 'scale(1.2)';
show.style.border = '1px solid goldenrod';
show.style.borderRadius = '5px'
show.classList.add("showing")} else {
